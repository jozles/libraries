...
v2.d  sleepDly corrigé -> le délai est juste à 1% ; la variable globale sleepTime est le temps "invisible" pendant le sleep
      ...
      .6 ajout gestion paramétrée powerLevel ; correction adcread0
...
/* !!!!!! changer de platformio.ini selon la MACHINE ('C'=due ou stm32 ; 'P' =328 !!!!! */
  ...
  #ifdef LORA
  #define RADIO_ADDR_LENGTH LORA_ADDR_LENGTH           // doit être == LORA_ADDR_LENGTH --- voir shconst
  #define MAX_PAYLOAD_LENGTH LORA_MAX_PAYLOAD_LENGTH   // doit être == LORA_MAX_PAYLOAD_LENGTH
  #define RADIO_TFR_DLY 4                              // mS délai entree tx conc et import sur perif après rx  (mesuré au scope)
  #endif
