...
void LoRaClass::addrWrite(uint8_t bid,byte* bidx)   // pour radioInit() inutile
{}

void LoRaClass::readStop()                         // pour radioInit() inutile
{}

void LoRaClass::flushRx()                          // à developper
{}

void LoRaClass::flushTx()                          // à developper
{}

void LoRaClass::printAddr(char* addr,char n)
{
  for(int j=0;j<LORA_ADDR_LENGTH;j++){Serial.print((char)addr[j]);}
...
void LoRaClass::setTxPower(int level, int outputPin)
{
  if (PA_OUTPUT_RFO_PIN == outputPin) {
    ...
    writeRegister(REG_PA_CONFIG, 0x70 | level);             // 0-14
  } else {
    // PA BOOST
    if (level > 17) {
      if (level > 20) {
    ...
    writeRegister(REG_PA_CONFIG, PA_BOOST | (level - 2));   // 0-15
  }
}
